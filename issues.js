...
function switchTab(tab) {
  ...
  tabNames.forEach(function (t) {
    ...
    if (t === tab) {
      ...
      btn.className = "btn btn-sm btn-ghost border border-gray-300 bg-white";
    }
  });
...
function buildCard(issue) {
  ...
  labelsArray.forEach(function (label) {
    ...
    labelsHTML +=
      ...
      style.classes + '">' + style.icon + label + '</span>';
  });
  ...
  card.innerHTML =
    '<div class="flex justify-between items-center mb-3">' +
      statusIcon +
      '<span class="text-xs font-semibold px-2 py-0.5 rounded ' + priorityStyle + '">' + priorityText + '</span>' +
    '</div>' +

    '<h3 class="font-bold text-gray-800 text-sm leading-snug mb-1">' + (issue.title || "Untitled") + '</h3>' +

    '<p class="text-gray-400 text-xs mb-3 line-clamp-2">' + (issue.description || "No description provided.") + '</p>' +

    '<div class="flex flex-wrap gap-1 mb-3">' + labelsHTML + '</div>' +

    '<div class="text-xs text-gray-400">' +
      '<p>#' + issueNum + ' by ' + author + '</p>' +
      '<p>' + dateText + '</p>' +
    '</div>';
...
function openModal(issue) {
  ...
  document.getElementById("modal-date").textContent = formatDate(issue.createdAt || issue.created_at);
  ...
  priorityBadge.className = "badge text-white font-semibold px-3 rounded " + getModalPriorityStyle(issue.priority);
  ...
  labelsArray.forEach(function (label) {
    ...
    span.className =
      "inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border font-medium " + style.classes;
...
function getPriorityStyle(priority) {
  ...
  if (p === "low")    return "bg-gray-100 text-gray-400";
...
function getLabelStyle(label) {
  if (!label) return { classes: "bg-gray-100 text-gray-500 border-gray-300", icon: "" };

  var l = label.toLowerCase();

  if (l === "bug") {
    return { classes: "bg-red-50 text-red-500 border-red-200",   icon: '<span class="mr-0.5">🐛</span>' };
  }
  if (l === "help wanted") {
    return { classes: "bg-green-50 text-green-600 border-green-200", icon: '<span class="mr-0.5">🙌</span>' };
  }
  if (l === "enhancement") {
    return { classes: "bg-purple-50 text-purple-600 border-purple-200", icon: '<span class="mr-0.5">✨</span>' };
  }
  if (l === "feature") {
    return { classes: "bg-blue-50 text-blue-600 border-blue-200", icon: '<span class="mr-0.5">🚀</span>' };
  }
  if (l === "documentation") {
    return { classes: "bg-yellow-50 text-yellow-600 border-yellow-200", icon: '<span class="mr-0.5">📄</span>' };
  }
